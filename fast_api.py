...
async def speech_scoring( context : str ,  audio_file: UploadFile = File(...) ,image_file: UploadFile = File(...) ):
...
@app.post("/paraphrasing_scoring/")
async def paraphrase_score( given_text : str,   audio_file: UploadFile = File(...) ):
    ...
    ''''''''''''''''''' GET THE CONTENT AND RELEVANCE SCORING '''''''''''''
    topic_text_embeddings = content_relevance_model.encode( given_text )
    transcription_text_embeddings = content_relevance_model.encode(transcription)
    relevance_scores = float(util.dot_score(topic_text_embeddings, transcription_text_embeddings).cpu()[0][0]) * 100
